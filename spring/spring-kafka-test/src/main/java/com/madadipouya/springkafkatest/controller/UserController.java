...
public class UserController {
    ...
    @GetMapping("/random-department")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "Create a user", description = "Creates a random user and write it to Kafka which is consumed by the listener")
    public void generateRandomDepartment() {
        kafkaProducer.writeToKafka(new Department(UUID.randomUUID().toString(), faker.name().name(), faker.code().asin()));
    }
    ...
    public void generateRandomString() {
        kafkaProducer.writeToKafkaString(faker.harryPotter().character());
    }
